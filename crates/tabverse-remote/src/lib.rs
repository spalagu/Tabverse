...
mod tests {
    ...
    async fn file_streams_are_available_to_current_app_viewers() -> Result<()> {
        ...
        struct TestFileSource(std::path::PathBuf);
        impl bridge::data_stream::RemoteFileSource for TestFileSource {
            fn open(
                &self,
                request: &FileReadRequest,
            ) -> Result<bridge::data_stream::OpenedRemoteFile> {
                ...
                Ok(bridge::data_stream::OpenedRemoteFile {
                    file,
        ...
        let tabverse_network::FileReadStart::File { head } = file_stream.response_start().await?
            ...
            bail!("Host file source refused a View reader");
        };
        assert_eq!(head.offset, 9);
        assert_eq!(head.length, file_body.len() as u64 - 9);
        let mut received = Vec::new();
        loop {
            let chunk = file_stream.read_chunk(64 * 1024).await?;
            if chunk.is_empty() {
                break;
            }
